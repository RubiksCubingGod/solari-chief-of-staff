...
## 2026-09-02 · hardening · round two

### Where the frontier is

- Repair `harden-request-context-guard` is done. Its first finish, `11c8518`, went out with a red gate: `typecheck:tests` rejected the new integration proof (TS2322, the step's literal return widened to `string`); fixed with `as const`. The second finish, `bc651d9`, and the third, `38b4282`, re-ran green (2 passed) and rules (17 passed) but their gates went red environmentally: launched without `TEST_DATABASE_URL`, the test-database ladder fell to the embedded rung, and parallel `initdb` under the gate's load blew the 120 s hook timeout (suites `skipped`, `page.goto: Timeout 30000ms`). The three playbooks suites that failed inside the gate passed alone (32 tests, 200 s). The fourth finish, `159dd8e`, ran with the documented server rung (`TEST_DATABASE_URL` on the 127.0.0.1:55432 throwaway cluster from the dashboard-read handoff): the database side was fine (ten `nah_test_*` databases, no embedded cluster) but the machine slept for about 87 minutes in the middle of the run, so eight files timed out. The fifth finish, `8f93ccc`, same rung, awake machine: gate green (1116 passed, 5 skipped; build and build:web clean).
- Finishing the task moved the frontier, so request `hardening-he358849b81584114` and attempt `attempt-r95a4c76549e14b0dae179d45e1d574d5` were invalidated by NAH. Recovered once with the documented cycle (`nah harden s5` refused with implementation required → `nah implement s5` auto-completed → `nah harden s5` adopted), which raised `hardening-h96fe1a426188fc4c` / `attempt-r52eacbb71ec54dc09050d00f5e015059`; the stale-receipt refreshes below moved the frontier again and invalidated that pair too, so the cycle ran a second time and the open request is now `hardening-h2508974bbd38d236` on attempt `attempt-rc823d00e440647f282312810e1ff06d0`. Every step ran with `--profile claude-only` (the portable default binds hardening to codex, which this session is not).
- Stale receipts on the done tasks were refreshed in this stage with bare `nah task finish` on the server rung, one task at a time. `task-state-machine` at `4041061` (green 11, rules 41, gate 1116 passed). `userio-gate` first at `27e8a41` with findings: its green and rules passed, but the gate failed one test in its own suite (`nothing was asked` in the late-answer test), which is finding 5 below and became the second repair; refreshed again after that repair at `1aabf80`. `guardrails` first at `a714c5c` with findings (green and rules passed; the gate failed two replay tests in the web task detail page, finding 6 below), then again at `7f42fbf`. `playbook-runner` at `5235bd7`. `nah implement`'s auto-complete re-runs nothing, so these were the only way to get current receipts before submitting.
- Second repair `harden-userio-delivery-wait` (test-only, `origin: hardening`) is done at `8127213`: red recorded deterministically (the first test fails on an empty channel with `expected [] to deeply equal`), green 6 passed, gate green (1116 passed, 5 skipped; build and build:web clean). Its first red attempt at 19:43Z ran while the test server was down (it had died at 19:40Z with a console-exit exception, `0xC000013A`, and was restarted with `pg_ctl`); the suite could not load, and NAH accepted that exit against the generic signature `1 failed`. That receipt is superseded, not erased: the red's signature was sharpened to the failing test's title (a new declaration digest), and the re-run at 19:47Z is the receipt that counts. Both are in `events.jsonl`.
- Round two is audited: no critical, high or medium gap. Next: submit the typed assurance result for the open request.

### Findings (round two, fresh dimensions)

Dimensions: the repair's own reach, escape hatches outside it, operational lifecycle, observability of the new stop, and (from the refresh evidence) determinism of the sprint's own proofs.

1. **Repair reach — satisfied.** playwright-core 1.62.1 sets `page.request = browserContext.request` in the Page constructor, and the context assigns that one `APIRequestContext` once in its own constructor, so popups and pages opened later share the shadowed object. `route.fetch` goes through `_innerFetch`, which the shadow does not touch, so the guard's own continuation path is unchanged (the guardrails suites pass in the gate). The stop is raised before the rejection, so `runGuarded`'s race settles on `stopped` even when a step catches the error, and the runner re-checks `guard.stop` after every step.
2. **Escape hatches — LOW, observation only.** A playbook could still reach `page.context().browser()?.newContext()` and get an unguarded context. Nothing under `packages/playbooks/src` imports playwright as a value or calls `browser()`/`newContext`; steps are typed to receive a `Page`. Closing it means wrapping the Browser object, which belongs with s9's hosted provider deciding what a session may create. No task.
3. **Operational lifecycle — satisfied.** The README worker row and `.env.example`'s `FAKEGYM_URL` landed at `11c8518`; the example's default matches `scripts/worker.mjs`.
4. **Observability — satisfied.** The task engine persists `{ reason, detail: stop }` on the `failed` transition with `cause: violation`; the dashboard's detail view model renders it as `running → failed (violation)` with the JSON detail on its own line, so `via: "request-context"`, the attempted URL and `reason: "unguarded"` are visible on the trail.
5. **Proof determinism — MEDIUM, repaired.** Surface: `packages/db/src/user-io.integration.test.ts`. Claim: the engine commits the question to the row and then hands it to the person (`askUser`, then `deliver`, by design and documented in `task-engine.ts`); three tests polled the row for `waiting_user` and read the scripted person's `asked` list in the same tick, so under gate load a poll can land in the gap and the whole gate goes red for nothing. It did, once, at `27e8a41`. Fingerprint: `user-io.integration.test.ts:waiting_user-then-io.asked`. Repair: a `delivered` helper waits for the channel, every post-`waiting_user` read goes through it, and the first test runs the person behind a 300 ms channel so the ordering is exercised on purpose rather than by scheduling luck. No production code changed; the task-engine, fakegym and runner suites read delivery state only after a terminal state that required the person's reply, so nothing else carries the pattern.

### Evidence findings (non-blocking, carried as recorded)

- Repair task receipts: red, green, rules and gate all current and green at `8f93ccc` (the three environmental red gates that precede it are preserved in the ledger as `task-findings-accepted` finishes at `bc651d9`, `38b4282` and `159dd8e`).
- Done-task receipts: all current and green after the refreshes above; `nah --json diagnostics s5` reports no stale receipt. The implementation-stage reds noted in round one (`task-lifecycle-green` at 08:10Z, `task-lifecycle-gate` at 05:22Z) are superseded by the `task-state-machine` refresh at `4041061`.
- Environmental receipts left in the ledger, all labelled: the `userio-gate` finish at `27e8a41` (red gate on the race, now repaired), the superseded first red attempt on `harden-userio-delivery-wait` (test server down), and the `guardrails` finish at `a714c5c` (red gate on finding 6).
- Environment note for whoever refreshes them: run every gate and Postgres proof with `TEST_DATABASE_URL=postgres://postgres:nahtest@127.0.0.1:55432/postgres` (the throwaway local server) and never concurrently with another session's `--coverage` run.

### Round three

Fresh dimensions after the second repair: determinism of sibling suites (audited above, nothing else reads in-memory delivery state before the reply that implies it), receipt currency (every done task refreshed on the server rung, listed in the frontier), gate determinism under parallel web suites (finding 6), and ledger hygiene (the environmental receipts are named rather than hidden). No critical, high or medium gap remains. Round cap is three, so a further gap would suspend rather than repair.

6. **Gate determinism under parallel web suites — LOW, evidence finding, no task.** Surface: `packages/web/src/task-detail-page.integration.test.ts` (covered by `replay-embed-page`). In the `guardrails` refresh at `a714c5c` the two replay tests failed: the dashboard's `/tasks/<id>/recording` route answered 404 for the rich and the corrupt task while the pages themselves rendered 200 with a player, and the player showed `could not be fetched: 404`. Alone, on the same tree and server rung, the suite passes (six tests; recordings 200, the missing one 502). The API answers that 404 only when the task row is not the caller's or has no recording, and each suite has its own `nah_test_*` database and its own auth stack, so a product cause was not found; the working hypothesis is interference between the in-process Next dev instances that six web suites run in parallel inside the one `packages/web` directory (each with its own `.next/instance-*`, but the same source tree and watcher), which is s4's harness and not this sprint's outcome. Recorded here and in DEFERRED.md rather than repaired: the round cap is reached, the failure is not attributable to this sprint's code, and the receipt that counts is the re-run.

### Resume

The typed `passed` result for request `hardening-h2508974bbd38d236` on attempt `attempt-rc823d00e440647f282312810e1ff06d0` goes to `nah lifecycle assurance s5` on stdin (profile `claude-only`). If this file still sits under `.nah/active`, that submission has not been accepted yet; under `.nah/done`, it has.
